...
inline void PtTo2Vertex(float3 vPt, float3 vOffs, float vTCy, float vNy, float2 vTexindTexcrdX, float2 a_vWorldTC, float3 a_vColor, float3 a_vBladeColor, float a_fNoise,  float a_fDissolve,
	inout TriangleStream< PSIn > TriStream)
{
    ...
    Vertex.vBladeColor = a_vBladeColor;
...
inline void Make4Pts( GSIn In, inout TriangleStream< PSIn > TriStream )
{
	...
	float3 vOffs = normalize(cross(vCamDir, vDir)) * SubTypes[In.uTypeIndex].vSizes.x * In.vPackedData.y;;
	float2 vWorldTC = (In.vPos0.xz) / g_fTerrRadius * 0.5 + 0.5;
    PtTo2Vertex(In.vPos3, 1.1*vOffs, 1.0, fNormalY, In.vPackedData.zw, vWorldTC, In.vColor, In.vBladeColor, In.fNoise, In.fDissolve, TriStream);
    /* Update offset */
    vDir = (In.vPos2 - In.vPos1);
    vOffs = normalize(cross(vCamDir, vDir)) * SubTypes[In.uTypeIndex].vSizes.x * In.vPackedData.y;;
    PtTo2Vertex(In.vPos2, 1.1*vOffs, 0.66, fNormalY, In.vPackedData.zw, vWorldTC, In.vColor, In.vBladeColor, In.fNoise, In.fDissolve, TriStream);
    /* Update offset */
    vDir = (In.vPos1 - In.vPos0);
    vOffs = normalize(cross(vCamDir, vDir)) * SubTypes[In.uTypeIndex].vSizes.x * In.vPackedData.y;;
    PtTo2Vertex(In.vPos1, 1.1*vOffs, 0.33, fNormalY, In.vPackedData.zw, vWorldTC, In.vColor, In.vBladeColor, In.fNoise, In.fDissolve, TriStream);
    /* Update offset */
//    vDir = float3(0.0, 1.0, 0.0);
//    vOffs = normalize(cross(vCamDir, vDir)) * SubTypes[In.uTypeIndex].vSizes.x * In.vPackedData.y;;
    PtTo2Vertex(In.vPos0, 1.1*vOffs, 0.0, fNormalY, In.vPackedData.zw, vWorldTC, In.vColor, In.vBladeColor, In.fNoise, In.fDissolve, TriStream);
...
inline void Make3Pts( GSIn In, inout TriangleStream< PSIn > TriStream )
{
    ...
    PtTo2Vertex(In.vPos3, 1.2*vOffs, 1.0, fNormalY, In.vPackedData.zw, vWorldTC, In.vColor, In.vBladeColor, In.fNoise, In.fDissolve, TriStream);
    /* Update offset */
    vDir = In.vPos1 - In.vPos0;
    vOffs = normalize(cross(vCamDir, vDir)) * SubTypes[In.uTypeIndex].vSizes.x * In.vPackedData.y;;
    PtTo2Vertex(In.vPos1, 1.2*vOffs, 0.33, fNormalY, In.vPackedData.zw, vWorldTC, In.vColor, In.vBladeColor, In.fNoise,  In.fDissolve, TriStream);
    /* Update offset */
   // vDir = float3(0.0, 1.0, 0.0);
   // vOffs = normalize(cross(vCamDir, vDir)) * SubTypes[In.uTypeIndex].vSizes.x * In.vPackedData.y;;
    PtTo2Vertex(In.vPos0, 1.2*vOffs, 0.0, fNormalY, In.vPackedData.zw, vWorldTC, In.vColor, In.vBladeColor, In.fNoise, In.fDissolve, TriStream);
...
//	vCamDir.y = 0.0;
	float3 vOffs = normalize(cross(vCamDir, vDir)) * SubTypes[In.uTypeIndex].vSizes.x * In.vPackedData.y;;
	//float fNormalY = max(normalize(vPos[3] - vPos[2]).y, 0.0);//normalize(cross(vDir, vOffs)).y;	
//	float fNormalY = max(vDir.y,0.0);
	float2 vWorldTC = (In.vPos0.xz) / g_fTerrRadius * 0.5 + 0.5;
...
//        vOffs[i] = sign(vDir.y)*normalize(cross(vCamDir, vDir)) * SubTypes[In.uTypeIndex].vSizes.x * In.vPackedData.y;
    ...
    [unroll]for (i = 0; i < 7; i++)
	{
	    PtTo2Vertex(vPts[i], vOffs[i], i * fInvNum, fNy, In.vPackedData.zw, vWorldTC, In.vColor, In.vBladeColor, In.fNoise,  In.fDissolve, TriStream);	    
	}   
