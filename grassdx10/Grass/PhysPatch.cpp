...
/*
    if (bp->brokenFlag == 1)
		...
		bp->g[j] = phi * props.vHardnessSegment[j-1];
...
float3 GetDw(float3x3 &T, float3x3 &R, float3 &fw, float3 &sum, float3 &w, float invJ, float segmentHeight, float Hardness, int j)
{
	...
	float fDemf = 0.0;
	float3 locw, locwM;
	D3DXVec3TransformCoord(&locw, &w, &T);
	D3DXVec3Cross(&locwM, &halfAxis, &locw);
	float3 phi = MakeRotationVector(R);
	float a[3] = {0.0025f, 0.0035f, 0.0045f};

	float fDemfx = 0.0f, fDemfz = 0.0f;
	fDemfx = a[j-1] * abs(fw.x);
	fDemfz = a[j-1] * abs(fw.z);
	g = phi * Hardness;
	if ((fw.x > 0.0f)&&(locwM.x - fw.x) > 0.0f) 
		fDemfx = 0.0;
	if ((fw.z > 0.0f)&&(locwM.z - fw.z) > 0.0f) 
		fDemfz = 0.0;

	Dw.x = (m_f.x - g.x - fDemfx * fw.x) * invJ;
	Dw.y = 0.0;
	Dw.z = (m_f.z - g.z - fDemfz * fw.z) * invJ;
	return Dw;
}
*/
float3 GetDw(float3x3 &T, float3x3 &R, float3 &fw, float3 &sum, float3 &w, float invJ, float segmentHeight, float Hardness, int j )
{
	...
	float3 locw, locwM; 
	D3DXVec3TransformCoord(&locw, &w, &T);
//	D3DXVec3Cross(&locwM, &locw, &halfAxis);
	D3DXVec3Cross(&locwM, &halfAxis, &locw);
	if (D3DXVec3Dot(&locwM, &fw) < 0.0)
		fDemf = j * 0.0025f*D3DXVec3Length(&fw);
	else
		fDemf = 0.0f;//0.00025f*D3DXVec3Length(&fw);
//	fDemf = 0.0f;
	//fDemf = 0.0;
	float3 phi = MakeRotationVector(R);
	g = phi * Hardness;
	//fDemf = 0.005f;
	fDemf = j *(0.0005f +0.0015f*D3DXVec3Length(&fw));
//	fDemf = j * 0.0015f*D3DXVec3Length(&fw);
	Dw = (m_f - g - fDemf * fw) * invJ;

/*	Dw.x = (m_f.x - g.x - fDemf * fw.x) * invJ;
	Dw.y = (m_f.y - 10.f*g.y - fDemf * fw.y) * 1.0f;
	Dw.z = (m_f.z - g.z - fDemf * fw.z) * invJ;

	float3x3 mR = MakeRotationMatrix(phi);
	float3 vphi = MakeRotationVector(mR);
*/
	return Dw;
}
...
//		float3 vphi = MakeRotationVector(Rres);
/*	float3 vphi = MakeRotationVector(T);
	if (D3DXVec3Length(&vphi) > 1.0)
		vphi.x = 1.0;
*/
}
...
//	float a[3]={0.5f, 0.5f, 0.5f};
    ...
    if (bp->brokenFlag > 1)
    {
        ...
        D3DXMATRIX tmp;
        D3DXMatrixMultiply(&tmp, &mRot, &mRotX);
...
void Phisics(PhysPatch::BladePhysData *bp, float3 &w, float dTime, Mesh *a_pMeshes[], const GrassPropsUnified &props)
{
	...
	for (int j = start_segment; j < NUM_SEGMENTS; j++)   
	{
		...
		float invJ = 0.5f*1.0f / oneThirdMMulLSqr;
		float h = props.vHardnessSegment[j-1];
		sum = g * props.vMassSegment[j-1] * bp->T[j][5] + w;
//		sum = g * props.vMassSegment[j-1] + w;

		float3 Dw = GetDw(bp->T[j], bp->R[j], bp->w[j], sum, w, invJ, bp->segmentHeight, h, j);
...
//		sum = g * props.vMassSegment[j-1] + w;
		float3 Dw_ = GetDw(Tres, Rres, w_, sum, w, invJ, bp->segmentHeight, h, j);
