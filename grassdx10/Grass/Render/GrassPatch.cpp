...
/* GrassPatch */
GrassPatch::GrassPatch( ID3D10Device *a_pD3DDevice, float a_fPatchSize, DWORD a_dwBladesPerSide )
{
...
ID3D10Device *GrassPatch::GetD3DDevicePtr( )
{
    return m_pD3DDevice;
}
...
GrassPatchLod0::GrassPatchLod0( ID3D10Device *a_pD3DDevice, float a_fPatchSize, DWORD a_dwBladesPerSide )
: GrassPatch(a_pD3DDevice, a_fPatchSize, a_dwBladesPerSide - a_dwBladesPerSide % 4 + 4)
{
    GeneratePatch();
    GenerateBuffers();
}
...
void GrassPatchLod0::GeneratePatch( )
{
    ...
    D3DXVECTOR3 vPivotPt = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
...
void GrassPatchLod0::GenerateBlade( DWORD *a_pCurVerticesIndex, D3DXVECTOR3 *a_pPivotPt, float a_fTransparency  )
{    
    ...
    float fLen = D3DXVec3Length(&m_pVertices[dwVertInd].vRotAxe);
//    m_pVertices[dwVertInd].vRotAxe *= (fRand(10.f)/fLen);
    m_pVertices[dwVertInd].vRotAxe *= (fRand(8.f)/fLen);
  m_pVertices[dwVertInd].vYRotAxe.y = 1.0f;
    m_pVertices[dwVertInd].vYRotAxe.x = 0.0;
    m_pVertices[dwVertInd].vYRotAxe.z = 0.0;
    fLen = D3DXVec3Length(&m_pVertices[dwVertInd].vYRotAxe);
    m_pVertices[dwVertInd].vYRotAxe *= (fRand(90.f)/fLen);
    ...
    std::pair<float3, float> colors[4] = {
        std::pair<float3, float>(float3(0.14f, 0.33f, 0.00f), 0.7f),
        std::pair<float3, float>(float3(0.33f, 0.42f, 0.06f), 0.1f),
        std::pair<float3, float>(float3(0.15f, 0.40f, 0.00f), 0.1f),
        std::pair<float3, float>(float3(0.34f, 0.60f, 0.00f), 0.1f),
    };
    ...
    for (UINT i = 0; i < sizeof(colors) / sizeof(colors[0]); ++i)
        ...
        {
            m_pVertices[dwVertInd].vColor = colors[i].first;
            break;
        }
        else
...
: GrassPatch(a_pBasePatch->m_pD3DDevice, a_pBasePatch->m_fPatchSize, a_pBasePatch->m_dwBladesPerSide / 2)
{
