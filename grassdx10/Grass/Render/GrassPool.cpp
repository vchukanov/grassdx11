...
GrassPool::GrassPool( ID3D10Device *a_pD3DDevice, ID3D10InputLayout *a_pAnimInputLayout, 
                     ID3D10Effect *a_pEffect, GrassPatch *a_pBasePatch, int a_iPatchCount, bool a_bUseLowGrass )
{
    ...
    const D3D10_INPUT_ELEMENT_DESC InputLayoutDesc[] =
    {
        { "R0MTX"         , 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 0 ,  D3D10_INPUT_PER_VERTEX_DATA, 0 },
        { "R0MTX"         , 1, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 16,  D3D10_INPUT_PER_VERTEX_DATA, 0 },
        { "R0MTX"         , 2, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 32,  D3D10_INPUT_PER_VERTEX_DATA, 0 },
        { "R0MTX"         , 3, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 48,  D3D10_INPUT_PER_VERTEX_DATA, 0 },
        { "R1MTX"         , 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 64,  D3D10_INPUT_PER_VERTEX_DATA, 0 },
        { "R1MTX"         , 1, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 80,  D3D10_INPUT_PER_VERTEX_DATA, 0 },
        { "R1MTX"         , 2, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 96,  D3D10_INPUT_PER_VERTEX_DATA, 0 },
        { "R1MTX"         , 3, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 112, D3D10_INPUT_PER_VERTEX_DATA, 0 },
        { "R2MTX"         , 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 128, D3D10_INPUT_PER_VERTEX_DATA, 0 },
        { "R2MTX"         , 1, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 144, D3D10_INPUT_PER_VERTEX_DATA, 0 },
        { "R2MTX"         , 2, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 160, D3D10_INPUT_PER_VERTEX_DATA, 0 },
        { "R2MTX"         , 3, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 176, D3D10_INPUT_PER_VERTEX_DATA, 0 },
        { "POSITION"      , 0, DXGI_FORMAT_R32G32B32_FLOAT   , 0, 192, D3D10_INPUT_PER_VERTEX_DATA, 0 },
        { "TRANSPARENCY"  , 0, DXGI_FORMAT_R32_FLOAT         , 0, 204, D3D10_INPUT_PER_VERTEX_DATA, 0 },
        { "BLADECOLOR"    , 0, DXGI_FORMAT_R32G32B32_FLOAT   , 0, 208, D3D10_INPUT_PER_VERTEX_DATA, 0 },
    };
    int iNumElements = sizeof( InputLayoutDesc ) / sizeof( D3D10_INPUT_ELEMENT_DESC );
...
int GrassPool::GetPatchCount( )
{
    return m_iPatchCount;
}

D3DXVECTOR3 GrassPool::GetPatchPos( int a_iPatchIndex )
{
    return D3DXVECTOR3(m_pPatches[a_iPatchIndex]->Transform._41,
...
bool GrassPool::FreePatch( D3DXVECTOR3 a_vPatchPos )
{
    /* looking for a match */
    int i;
    for (i = 0; i < m_iPatchCount; i++)
    {
		...
		if (m_pPatches[i]->bIsDead)
			continue;
        if (D3DXVec3Alike(GetPatchPos(i), a_vPatchPos))
        {
...
int GrassPool::GetPatchIndex( D3DXVECTOR3 a_vPatchPos )
{
    /* looking for a match */
    int i;
    for (i = 0; i < m_iPatchCount; i++)
...
bool GrassPool::TakePatch( D3DXMATRIX a_mTransform, float a_fLifeTime, UINT a_uMeshIndex )
{    
    D3DXVECTOR3 vNewPos(a_mTransform._41,
        a_mTransform._42,
        a_mTransform._43);
    /* looking for a match */
    int i;
    for (i = 0; i < m_iPatchCount; i++)
    {
        ...
        if (m_pPatches[i]->bIsDead)
            continue;
        if (D3DXVec3Alike(GetPatchPos(i), vNewPos))
        {
...
bool GrassPool::TakePatch( D3DXMATRIX a_mTransform, bool a_bInFirstLod )
{    
    D3DXVECTOR3 vNewPos(a_mTransform._41,
        a_mTransform._42,
        a_mTransform._43);
...
/*
	 for (int i = 0; i < m_iPatchCount; i += 1)
    {   
        if (m_pPatches[i]->bIsDead)
            continue;
        ...
        if (collision)
        {
            D3DXVECTOR4 sphere = a_pMeshes[m_pPatches[i]->uMeshIndex]->GetPosAndRadius();
            sr = sphere.w;
            sp = *(float3*)(float*)(&sphere);
        }
...
void GrassPool::Render( bool a_bShadowPass )
{
    ...
    /* Only physics */
    m_pD3DDevice->IASetInputLayout(m_pPhysInputLayout);    
	if (m_bUseLowGrass)
    {
        m_pLowGrassPhysPass->Apply(0);
        for (i = 0; i < m_iPatchCount; i++)
        {
            if (!m_pPatches[i]->bIsDead)
            {             
                m_pPatches[i]->Patch.IASetPhysVertexBuffer0();
                m_pD3DDevice->Draw(m_pPatches[i]->Patch.PhysVerticesCount(), 0);
            }
        }
    }
    m_pRenderPhysPass->Apply(0);
    for (i = 0; i < m_iPatchCount; i++)
    {
        if (!m_pPatches[i]->bIsDead)
        {
	...
	m_pD3DDevice->IASetInputLayout(m_pAnimInputLayout);    
    if (m_bUseLowGrass)
    {
        m_pLowGrassAnimPass->Apply(0);
        for (i = 0; i < m_iPatchCount; i++)
        {
            if (!m_pPatches[i]->bIsDead)
            {             
                m_pPatches[i]->Patch.IASetAnimVertexBuffer0();
                m_pD3DDevice->Draw(m_pPatches[i]->Patch.AnimVerticesCount(), 0);
            }
        }
    }
	
    m_pRenderAnimPass->Apply(0);
    for (i = 0; i < m_iPatchCount; i++)
    {
        if (!m_pPatches[i]->bIsDead)
        {
