...
int WINAPI wWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow )
{
    ...
    DXUTCreateWindow( L"Grass Demo 11.02.11" );
...
void InitApp()
{
    ...
    g_HUD.AddButton( IDC_TOGGLE_WIREFRAME, L"Toggle wire-frame (F4)", 25, iY += iYInc, 125, 22, VK_F4 );
    iYInc = 18;
    ...
    g_HUD.AddSlider( IDC_GRASS_WIND_FORCE_SLYDER, 20, iY += iYInc, 135, 22, 0, 10000, ( int )( g_fWindStrength * 10000 ) );
        ...
        case IDC_CAMERA_TYPE:
        {
            ...
            switch (value)
            {
		     case CAMERA_NORMAL:
                ...
                // Enable wind
                if (g_fWindStrength == 0.0f)
                {
                    g_HUD.GetSlider(IDC_GRASS_WIND_FORCE_SLYDER)->
                        SetValue((int)(g_fWindStrengthDefault * 10000.0f));
                    OnGUIEvent(0, IDC_GRASS_WIND_FORCE_SLYDER,
                        g_HUD.GetSlider(IDC_GRASS_WIND_FORCE_SLYDER), NULL);
                }
                break;
            case CAMERA_TERRAIN:
                if (g_Camera != NULL)
...
//                        g_fCarFrontWidth, g_fCarHeight, g_fCarLength, 0.05f);
                    ...
                    // Disable wind                    
                    g_HUD.GetSlider(IDC_GRASS_WIND_FORCE_SLYDER)->SetValue(0);
                    OnGUIEvent(0, IDC_GRASS_WIND_FORCE_SLYDER,
                        g_HUD.GetSlider(IDC_GRASS_WIND_FORCE_SLYDER), NULL);
                }
            ...
            g_Camera->SetViewParams( &g_vCameraEyeStart, &g_vCameraAtStart );
                                      ...
                                      void* pUserContext )
{
    ...
    GetGlobalStateManager().SetDevice(pd3dDevice);
    ...
    CurrentRasterizerState.CullMode = D3D10_CULL_NONE;
    CurrentRasterizerState.FillMode = D3D10_FILL_WIREFRAME;
...
static float3 vCarDir = float3(0.0f, 0.0f, 1.0f);
...
//		fFi = (float)M_PI * sinf(fTimeEmul);
	...
	//        pos.z -= cosf(g_fTime*0.1f) * 20.0f;
			...
			pos.x += vCarDir.x * 0.1f;
			pos.z += vCarDir.z * 0.1f;
...
//			g_fCarRotVel += 0.2f*g_fCarRotAccel*a_fElapsedTime;
			g_fCarRotVel *= d;
			if (g_fCarRotVel > 7.0f*g_fCarVelocity*g_fCarRotMaxVel) g_fCarRotVel = 7.0f*g_fCarVelocity*g_fCarRotMaxVel;
			if (g_fCarRotVel < 7.0f*g_fCarVelocity*g_fCarRotMinVel) g_fCarRotVel = 7.0f*g_fCarVelocity*g_fCarRotMinVel;
//			if (g_fCarRotVel > 7.0f*g_fCarRotMaxVel) g_fCarRotVel = 7.0f*g_fCarRotMaxVel;
//			if (g_fCarRotVel < 7.0f*g_fCarRotMinVel) g_fCarRotVel = 7.0f*g_fCarRotMinVel;
		}
		else
		{
        ...
        float3x3 mrot;
        D3DXVec3TransformCoord(&g_vCarDir,
            &g_vCarDir, D3DXMatrixRotationY(&mrot, g_fCarRotVel));
/*
        D3DXMATRIX mRot;
			D3DXMatrixRotationY(&mRot, fFi);
			D3DXVec3TransformCoord(&vCarDir, &vCarDir, &mRot);
			*/
		g_vCarDir.y = 0.0;
			D3DXVec3Normalize(&g_vCarDir, &g_vCarDir);
        ...
        for (UINT i = 0; i < g_fNumOfMeshes; i++)
        {
            pos.x += g_vCarDir.x * g_fCarVelocity;
            pos.z += g_vCarDir.z * g_fCarVelocity;
            //pos.x += g_vCarDir.x * 0.01f;
            //pos.z += g_vCarDir.z * 0.01f;
            g_pMeshes[i]->SetPosAndRadius(pos);
        }
    }
}
